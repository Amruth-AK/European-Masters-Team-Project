...
# Sentinel for features not applicable to a given column-type combination
SENTINEL = -10.0
...
SCHEMA_INTERACTION = [
    ...
    'pearson_corr',                   # num+num only, else SENTINEL
    'spearman_corr',                  # num+num only, else SENTINEL
    'mutual_info_pair',               # MI between the two columns (all types)
    'mic_score',                      # Maximal Information Coefficient (all types)
    'scale_ratio',                    # max(std)/min(std), num+num only, else SENTINEL
    ...
    'sum_target_corr',                # num+num: sum of |spearman with target|, else SENTINEL
    'abs_diff_target_corr',           # num+num: abs diff of target correlations, else SENTINEL
...
def apply_interaction_transform(X_train, X_val, y_train, col_a, col_b, method):
    ...
    try:
        if method == 'product_interaction':
            ...
            le = LabelEncoder()
            le.fit(pd.concat([combined_train, combined_val]))
            X_train[new_col] = le.transform(combined_train)
            X_val[new_col] = le.transform(combined_val)
...
def _column_target_corr(series, y_numeric):
    """Spearman correlation with target (numeric only)."""
...
def get_pair_meta_features(col_a_series, col_b_series, y, imp_a, imp_b):
    ...
    Features only applicable to certain type combinations use SENTINEL (-10).
    """
    ...
    # ---- Pure pairwise features (require both columns) ----
    ...
    if n_numerical == 2:
        try:
            ...
            if clean.sum() > 10:
                meta['pearson_corr'] = float(abs(
                    col_a_series[clean].corr(col_b_series[clean])
                ))
            else:
    ...
    if n_numerical == 2:
        ...
        meta['spearman_corr'] = SENTINEL
    
    # Mutual information between the two columns
    try:
        enc_a = _encode_for_mi(col_a_series)
        enc_b = _encode_for_mi(col_b_series)
        # MI(A, B) using B as "target"
    ...
    # Maximal Information Coefficient approximation
    # Uses binned MI as a proxy (no minepy dependency needed).
    # Bins both columns into quantiles, then computes MI on the discretized version.
    try:
        n = len(col_a_series)
        n_bins = min(20, max(5, int(n ** 0.4)))  # adaptive bin count
        
        # Encode columns into bins
        if a_is_num:
        ...
        # MI(A_binned, B_binned) normalized to [0, 1]
        ...
        meta['mic_score'] = float(mi_binned / max(h_b, 1e-10)) if h_b > 0 else 0.0
        meta['mic_score'] = min(meta['mic_score'], 1.0)  # clip to [0, 1]
    except Exception:
        meta['mic_score'] = 0.0
    ...
    # ---- Combined individual stats (order-invariant) ----
    ...
    # Target correlations (num+num only, else SENTINEL)
    ...
    if n_numerical == 2:
        ...
        meta['sum_target_corr'] = SENTINEL
        meta['abs_diff_target_corr'] = SENTINEL
    
    # MI with target (works for all types)
...
def collect_interaction_features(X, y, dataset_name="unknown",
    ...
    for pair_idx, (col_a, col_b, methods) in enumerate(all_pairs):
        ...
        for method in methods:
            ...
            # Individual stats
            indiv_delta = compute_delta(indiv_base_score, indiv_score) if indiv_score else np.nan
