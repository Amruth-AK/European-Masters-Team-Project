...
Usage:
    python train_hp_meta_model.py --hp-db ./hp_tuning_output/hp_tuning_db.csv --output-dir ./hp_meta_model
...
SCORER_FEATURES = DATASET_FEATURES + ALL_HP_FEATURES + ['task_type_encoded']

# Feature set for direct predictor (dataset only)
PREDICTOR_FEATURES = DATASET_FEATURES + ['task_type_encoded']
...
class HPDataPreparator:
    ...
    def prepare(self, df: pd.DataFrame, top_k=3):
        ...
        all_feature_cols = list(set(
            DATASET_FEATURES + HP_PARAM_NAMES + HP_DERIVED_FEATURES + ['task_type_encoded']
        ))
...
def train_hp_meta_model(hp_db_path: str, output_dir: str = './hp_meta_model',
    ...
    # --- Archetypes ---
    archetype_data = compute_archetypes(df)
