...
3. LATIN HYPERCUBE SAMPLING for HP configurations:
   ...
   - 80 sampled configs + 1 default = 26 evaluations per dataset
...
def _generate_hp_configs(n_samples=80, seed=42):
    """
...
class HPTuningCollector:
    ...
    2. Evaluates ~61 HP configurations via repeated cross-validation
    ...
    def __init__(self, task_type='classification', n_folds=5, n_repeats=3,
                 n_hp_configs=80, output_dir='./hp_tuning_output',
                 time_budget_seconds=3600, n_jobs=-1, seed=42,
                 sampler='optuna', n_startup_trials=None, worker_id=None):
        ...
        if worker_id is not None:
            ...
            self.log_file = os.path.join(worker_log_dir, f'worker_{worker_id:05d}.log')
            self.error_log_file = os.path.join(worker_log_dir, f'worker_{worker_id:05d}_errors.log')
        else:
    ...
    def _prepare_features(self, X):
        ...
        # Deduplicate after sanitization (e.g. "col[0]" and "col_0_" → same name)
        seen = {}
        new_cols = []
        ...
        for train_idx, val_idx in cv.split(X, y if self.task_type == 'classification' else None):
            ...
            try:
                if self.task_type == 'classification':
                    ...
                    # Filter eval_set to avoid unseen-label crash on rare classes
                    train_classes = set(y_train.unique())
                    eval_mask = y_val.isin(train_classes)
                    if eval_mask.sum() >= 10:   # enough rows to be useful for early stopping
                        es_eval_set = [(X_val[eval_mask], y_val[eval_mask])]
                    else:
                        es_eval_set = []   # skip early stopping for this fold

                    fit_kwargs = dict(callbacks=[early_stopping(stopping_rounds=50, verbose=False)]) \
                                if es_eval_set else {}

                    model.fit(X_train, y_train,
                            eval_set=es_eval_set or None,
                            **fit_kwargs)
                    ...
                    # Guard against NaN predictions from extreme HP configs
                    nan_mask = ~np.isnan(preds)
                    if nan_mask.sum() < 10:
                        self.log_error("Too many NaN predictions in regression fold — skipping fold")
                        continue
                    preds = preds[nan_mask]
                    y_val_clean = y_val.values[nan_mask]

                    # Metrics (all use the cleaned arrays)
                    fold_metrics['rmse'].append(np.sqrt(mean_squared_error(y_val_clean, preds)))
                    fold_metrics['mae'].append(mean_absolute_error(y_val_clean, preds))
                    fold_metrics['r2'].append(r2_score(y_val_clean, preds))

                    # MAPE (avoid division by zero)
                    mask = y_val_clean != 0
                    if mask.sum() > 0:
                        mape = np.mean(np.abs((y_val_clean[mask] - preds[mask]) / y_val_clean[mask])) * 100
...
def run_single_task(task_entry, output_dir, n_folds=5, n_repeats=3,
                    n_hp_configs=80, time_budget=3600, worker_id=None, seed=42,
    ...
    # ── Create dirs FIRST so the SIGTERM handler can safely write ──
    ...
    # ── Now safe to register the handler ──
    def _sigterm_handler(signum, frame):
        print(f"Worker {worker_id}: SIGTERM received — writing 'killed' checkpoint")
        try:
            with open(checkpoint_file, 'w') as f:
                json.dump({'task_id': task_id, 'dataset_id': dataset_id,
                           'status': 'killed', 'worker_id': worker_id}, f)
        except Exception:
            pass
        sys.exit(1)
    ...
    try:
        ...
        # --- Adaptive n_repeats based on dataset size (post-reduction) ---
        if X.shape[0] >= 500_000:
            effective_repeats = 1
        elif X.shape[0] >= 100_000:
            effective_repeats = 2
        else:
            effective_repeats = n_repeats
        if effective_repeats != n_repeats:
            print(f"  Large dataset ({X.shape[0]:,} rows) → reducing n_repeats: {n_repeats} → {effective_repeats}")

        # --- Run collector ---
        collector = HPTuningCollector(
            task_type=task_type, n_folds=n_folds, n_repeats=effective_repeats,
            n_hp_configs=n_hp_configs, output_dir=output_dir,
            time_budget_seconds=time_budget, seed=seed,
            sampler=sampler, n_startup_trials=n_startup_trials, worker_id=worker_id)
...
def run_pipeline(output_dir='./hp_tuning_output', task_list_file=None,
                 n_folds=5, n_repeats=3, n_hp_configs=80,
...
if __name__ == "__main__":
    ...
    parser.add_argument('--n_hp_configs', type=int, default=80,
                        help='Number of HP configs to evaluate (+ 1 default). Default: 80')
