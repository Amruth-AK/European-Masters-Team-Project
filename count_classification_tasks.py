...
def main():
    ...
    parser.add_argument('--task_list', default='/work/inestp05/Final/task_list.json')
    ...
    if max_idx < 1000:
        ...
        print(f"  --export=COLLECTOR=numerical,OUTPUT_DIR=/work/inestp05/Final/output_numerical \\")
        print(f"  run_collector.sh")
        print()
        print(f"sbatch --array=0-{max_idx}%50 --job-name=cat_collect \\")
        print(f"  --export=COLLECTOR=categorical,OUTPUT_DIR=/work/inestp05/Final/output_categorical \\")
        print(f"  run_collector.sh")
        print()
        print(f"sbatch --array=0-{max_idx}%50 --job-name=int_collect \\")
        print(f"  --export=COLLECTOR=interaction,OUTPUT_DIR=/work/inestp05/Final/output_interactions \\")
        print(f"  run_collector.sh")
    else:
        ...
        for collector, short, outdir in [
            ...
            while offset <= max_idx:
                ...
                print(f"  --export=COLLECTOR={collector},OUTPUT_DIR=/work/inestp05/Final/{outdir},OFFSET={offset} \\")
                print(f"  run_collector.sh")
                offset += batch_size + 1
                batch_num += 1
    print()
    ...
    print("=" * 70)
    print()
    print("python merge_collector_results.py --output_dir /work/inestp05/Final/output_numerical --prefix numerical_transforms")
    print("python merge_collector_results.py --output_dir /work/inestp05/Final/output_categorical --prefix categorical_transforms")
    print("python merge_collector_results.py --output_dir /work/inestp05/Final/output_interactions --prefix interaction_features")
