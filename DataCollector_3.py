...
class MetaDataCollector:
    """
    Meta-Learning Data Collector v7 ("AutoFeat + FeatureWiz Integration")
    ...
    # =========================================================================
    # FIXED CSV SCHEMA Ã¢â‚¬â€ every row MUST have exactly these columns in this order.
    ...
    def __init__(self, task_type='classification', n_folds=5, n_repeats=3, 
        ...
        # v5: Noise-floor calibration (set during collect after null intervention)
        self._null_delta = 0.0
        self._null_std = 1e-4  # Avoid division by zero (clamped minimum)
        ...
        # Simpler params for individual column evaluation (few features Ã¢â€ â€™ simpler model)
    ...
    def _select_columns_stratified(self, X, y, base_feature_importance):
        ...
        self.log(f"Column selection: {n_cols} columns Ã¢â€ â€™ cap at {cap}")
    ...
    def get_column_meta(self, col_series, y_target, baseline_importance=None):
        ...
        if is_num:
            ...
            if len(clean_col) >= 3:
                try:
                    sample = clean_col.sample(min(100000, len(clean_col)), random_state=42)
    ...
    def _apply_intervention(self, X_train, X_val, y_train, col_to_transform, 
        ...
        try:
            if method == 'impute_median':
                ...
                # ADDS sin/cos columns Ã¢â‚¬â€ keeps original for tree histogram splits.
                ...
                # (a * b) / c â€” useful for rate-like derived features
                eps = 1e-5
                ...
                # (a - b) / c â€” normalized difference, e.g. (income - expenses) / household_size
    ...
    def evaluate_with_intervention(self, X, y, col_to_transform=None, col_second=None, 
        ...
        for train_idx, val_idx in cv.split(X):
            ...
            # Apply intervention â€” unpack (success, X_train, X_val) tuple
            ...
            try:
                if self.task_type == 'classification':
                    ...
                    # v7: Use RÂ² as primary regression score (scale-invariant, higher=better)
    ...
    def _get_delta(self, base_score, new_score):
        # Both AUC (classification) and RÂ² (regression) are higher-is-better
        return new_score - base_score

    def _normalize_delta(self, delta, base_score):
        # Both AUC and RÂ² are in [0,1]-ish units; delta * 100 = percentage points
        return delta * 100
    ...
    def _check_ceiling_effect(self, baseline_score):
        if self.task_type == 'classification':
            ...
            # RÂ² is primary regression metric: 1.0 = perfect, higher = better
            if baseline_score >= 0.995:
                return True, False, f"ceiling_hard (RÂ²={baseline_score:.5f} >= 0.995)"
            elif baseline_score >= 0.98:
                return False, True, f"ceiling_soft (RÂ²={baseline_score:.5f} >= 0.98)"
    ...
    TEMPORAL_PATTERNS = [
        ...
        # Minute: 0-59 (require full word 'minute' Ã¢â‚¬â€ 'min' is too ambiguous: min_temperature, min_val)
        (r'(?i)(?:^|[_\W])(?:minute)(?:$|[_\W\d])',
         lambda mn, mx: 0 <= mn and mx <= 59, 'minute', 60),
        # Second: 0-59 (require full word 'second' Ã¢â‚¬â€ 'sec' is too ambiguous: section, security)
    ...
    # the row-to-bin mapping is equivalent Ã¢â‚¬â€ just with different split thresholds.
    #
    ...
    INDIVIDUAL_INVARIANT_METHODS = {
        'frequency_encoding',   # re-encodes cat Ã¢â€ â€™ frequency floats (injective)
        'target_encoding',      # re-encodes cat Ã¢â€ â€™ target-smoothed floats (injective with smoothing)
        'hashing_encoding',     # re-encodes cat Ã¢â€ â€™ hash integers (injective mod collisions)
        'log_transform',        # monotonic transform Ã¢â€ â€™ same histogram partitions
        'sqrt_transform',       # monotonic transform Ã¢â€ â€™ same histogram partitions
    ...
    def _should_test_method(self, method, col_meta):
        ...
        v4 tightened gates Ã¢â‚¬â€ log/sqrt/poly/binning rarely help with LightGBM:
        ...
        - quantile_binning: only when outlier > 0.30 AND range_iqr > 15 AND is_multimodal
          (almost never helps Ã¢â‚¬â€ only test with very strong evidence of pathological distribution)
        ...
        if method in self.GATED_METHODS:
            ...
            if method == 'log_transform':
                ...
                # v4: stricter Ã¢â‚¬â€ need strong skew + non-negative + some outliers
                if not (skew > 2.5 and min_val >= 0 and outlier > 0.10):
                    return False, "property_gate: skew<=2.5 or min<0 or outlier<=0.10"
                    
            elif method == 'quantile_binning':
                # v4: extremely strict Ã¢â‚¬â€ almost never helps with LightGBM
    ...
    def _update_method_tracker(self, method, improved, n_selected_cols):
        ...
        - Rarely-useful methods (log, sqrt, poly_square, quantile_binning):
          After 5 columns with 0 improvements Ã¢â€ â€™ disable.
        - Other methods: After 20% of columns (min 10) with 0 improvements Ã¢â€ â€™ disable.
        """
        ...
        if tracker['tested'] >= min_trials and tracker['improved'] == 0:
            ...
            self.log(f"    Ã¢Å¡Â¡ Adaptive early-stop: {method} disabled after {tracker['tested']} "
    ...
    def _compute_calibrated_delta(self, delta):
        """
        Calibrate a raw delta against the noise floor from the null intervention.
        
        calibrated = (delta - null_delta) / null_std
        
        A calibrated delta of 2.0 means "2 standard deviations above what we'd 
        expect from random retraining variance alone."
        """
        if np.isnan(delta) or self._null_std < 1e-4:
            return np.nan
    ...
    def _parse_autofeat_feature_name(feat_name, known_columns):
        ...
        op = 'unknown'
        if '**2' in feat_name or '² ' in feat_name:
    ...
    def _autofeat_discover_pairs(self, X, y, max_rows=5000, max_cols=30):
        ...
        AutoFeat generates O(n²) candidate features from polynomial and
    ...
    def collect(self, df, target_col, dataset_name="unknown"):
        ...
        Pipeline:
        1. ID detection Ã¢â€ â€™ drop
        2. Strategic pruning (constant, near-constant, zero-var, r>0.99) Ã¢â€ â€™ drop
        3. Target leakage detection (corr/MI/PPS > 0.95 with target) Ã¢â€ â€™ drop
        ...
        # Phase 0.1: Deduplication Ã¢â‚¬â€ track (method, col_a, col_b) tuples already tested
        # For commutative ops (product, addition, abs_diff), sort col names so AÃƒâ€”B == BÃƒâ€”A
        ...
        if leaky:
            self.log(f"Ã¢Å¡Â Ã¯Â¸Â  Target leakage detected in {len(leaky)} columns:")
        ...
        if should_skip:
            self.log(f"Ã¢Å¡Â Ã¯Â¸Â  SKIPPING: {ceiling_reason}", "WARNING")
            return pd.DataFrame()
        ...
        # Phase 2.5: relative_headroom â€” how much room for improvement exists
        # Both AUC (classification) and RÂ² (regression) are bounded at 1.0
        ...
        # 11. NULL INTERVENTION + v7 NOISE FLOOR (3 repetitions for stability)
        self.log("Testing null intervention (3 reps for noise floor)...")
        null_deltas = []
        null_fold_stds = []
        null_score_final = None
        null_folds_final = None
        for _null_rep in range(3):
            null_score, null_std_rep, null_folds = self.evaluate_with_intervention(X_full, y, method='null_intervention')
            if null_score is not None:
                d = self._get_delta(base_score, null_score)
                null_deltas.append(d)
                if null_folds is not None and baseline_fold_scores is not None:
                    fold_diffs = np.array(null_folds) - np.array(baseline_fold_scores)
                    null_fold_stds.append(np.std(fold_diffs, ddof=1))
                # Keep last successful result for logging
                null_score_final = null_score
                null_folds_final = null_folds
        
        if null_deltas:
            self.n_tests_performed += 1
            # Use median for robustness against outlier retraining runs
            self._null_delta = float(np.median(null_deltas))
            if null_fold_stds:
                # v7b: Clamp null_std to 1e-4 (0.0001) minimum to prevent
                # calibrated_delta explosion when model is deterministic.
                self._null_std = max(float(np.median(null_fold_stds)), 1e-4)
            
            ds_meta['null_delta'] = self._null_delta
            ds_meta['null_std'] = self._null_std
            self.log(f"  Noise floor ({len(null_deltas)} reps): null_delta={self._null_delta:.6f}, null_std={self._null_std:.6f}")
            
            self._validate_and_append(
                ds_meta=ds_meta,
                column_name='NULL_INTERVENTION', method='null_intervention',
                delta=self._null_delta, delta_normalized=self._normalize_delta(self._null_delta, base_score),
                absolute_score=null_score_final, is_interaction=False,
        ...
        for col_idx, col in enumerate(selected_cols):
            ...
            if is_date:
                ...
                # Skip log/sqrt/poly/binning Ã¢â‚¬â€ meaningless for cyclical data.
                # Keep impute_median/missing_indicator for null handling.
                methods = ['cyclical_encode', 'impute_median', 'missing_indicator']
                self.log(f"    Ã°Å¸â€¢Â Detected temporal component: {temporal_type} (period={temporal_period})")
            ...
            for top_col in top_global_cols:
                ...
                if both_numeric:
                    ...
                    # Reverse: top_col is cat, col is num Ã¢â€ â€™ group_mean with top as grouper
        ...
        # =====================================================================
        # 14. CROSS-COLUMN INTERACTIONS (top features Ãƒâ€” top features)
        # =====================================================================
        ...
        # v7: AUTOFEAT INTERACTION PAIR DISCOVERY
        # =====================================================================
        # AutoFeat uses polynomial feature generation + L1 selection to find
        # which feature combinations are predictive. We parse the surviving
        # features back into column pairs and feed them into the interaction
        # testing pipeline as additional candidates.
        autofeat_pairs = []
        autofeat_singles = []
        ...
        if len(top_arithmetic) >= 2 and not self._is_over_budget():
        ...
        if autofeat_pairs:
            ...
            if autofeat_singles:
                self.log(f"AutoFeat: {len(autofeat_singles)} single-column transforms identified "
                         f"(logged for reference: {autofeat_singles[:5]})")
    ...
    def _is_interaction_duplicate(self, method, col_a, col_b, col_c=None):
        ...
        For commutative operations, sort column names so AÃƒâ€”B == BÃƒâ€”A.
        ...
        if key in self._tested_interactions:
            return True  # duplicate Ã¢â‚¬â€ skip
        self._tested_interactions.add(key)
        return False  # new Ã¢â‚¬â€ proceed
...
# Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ Curated temporal/date-heavy datasets for testing temporal detection Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
...
TEMPORAL_EXTRA_DATASETS = [
    ...
    (42730, 'Online_Shoppers_Intention', 
     'Month Ã¢â€ â€™ cyclical/cat; Weekend; classification'),

    # Avocados (Regression)
    # Features: Date (string)
    (42722, 'Avocado_Sales', 
     'Date Ã¢â€ â€™ date_extract; regression'),
    ...
    (1487, 'Ozone_Level_8hr', 
     'Date Ã¢â€ â€™ date_extract; classification'),

    # Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ EXTRA: DateTime String Heavy Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
    
    # Bitcoin Heist (Classification)
    # Features: only has 'year' and 'day' integers, but good for simple cyclical
    (42617, 'Bitcoin_Heist', 
     'day Ã¢â€ â€™ cyclical; classification'),
...
def _resolve_task_for_dataset(dataset_id, preferred_type=None):
    ...
    try:
        ...
        print(f"  Ã¢Å¡Â Ã¯Â¸Â  Error resolving task for dataset {dataset_id}: {e}")
...
def _infer_task_type(y):
    ...
    Heuristic ladder:
    1. Non-numeric target Ã¢â€ â€™ classification (always)
    2. Ã¢â€°Â¤ 2 unique values Ã¢â€ â€™ classification (binary)
    3. Float (fractional values) with many uniques Ã¢â€ â€™ regression
    4. Integer-like values: check unique_ratio (nunique / n_rows)
    - unique_ratio < 0.05 Ã¢â€ â€™ classification (few classes, many repeats)
    - nunique Ã¢â€°Â¤ 200 AND unique_ratio < 0.10 Ã¢â€ â€™ classification (medium class count)
    - Otherwise Ã¢â€ â€™ regression
    """
    ...
    # Float values with many unique Ã¢â€ â€™ regression
...
def _smart_size_reduction(X, y, task_type, cell_limit=100_000_000):
    ...
    if drop_cols:
        ...
        log_parts.append(f"pruned {len(drop_cols)} junk cols ({original_shape[1]}Ã¢â€ â€™{X.shape[1]})")
    
    if X.shape[1] == 0:
        return X, y, "all columns pruned Ã¢â‚¬â€ skip"
    ...
    log_parts.append(f"stratified sampled {original_rows:,}Ã¢â€ â€™{len(X):,} rows")
...
def run_openml_pipeline(output_dir='./meta_learning_output',
    ...
    print("=" * 80)
    print("OPENML META-LEARNING PIPELINE v5 (Local Sequential)")
    print("=" * 80)
    ...
    for entry in task_list:
        ...
        try:
            ...
            if raw_cells > 100_000_000:
                print(f"  Large dataset ({X.shape}, {raw_cells:,} cells) â€” applying smart reduction...")
                X, y, reduction_msg = _smart_size_reduction(X, y, task_type, cell_limit=100_000_000)
                print(f"  â†’ {reduction_msg}")
                if X.shape[1] == 0:
                    print("  No columns survived pruning â€” skipping")
                    processed_tasks.add(ckpt_key)
                    tasks_skipped += 1
                    continue
